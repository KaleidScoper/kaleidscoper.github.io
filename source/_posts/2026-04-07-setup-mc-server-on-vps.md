...
伴随 26.1.1 更新实装，本文在 2026 年 4 月 7 日随之做出了适当更新，更正了关于游戏存档格式的说明。原文于 2024 年 10 月 2 日发布。
...
<b>A：</b>官方近期调整了游戏存档的文件结构，将地图数据与存档的其他数据分开了。
...
换句话说，曾经主世界的地图数据是直接存放在存档根目录的，和下界、末地的地图数据不同级，看上去很混乱。在新的结构中，存档文件夹根目录新设立了一个“维度”文件夹，三个维度的地图数据现在平级了。

理论上，对于旧格式存档，只需要新建这个不存在的维度文件夹后，将原本在 DIM-1、DIM1 和 world 内的各四个地图数据，按新规则放到维度文件夹的指定位置，就能正常打开。
