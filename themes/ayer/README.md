...
Ayer —— 马来语中"水"的意思，西班牙语中"昨日"的意思。原作者：[Eric-Shen](https://github.com/Shen-Yu)。
...
```bash
git clone https://github.com/KaleidScoper/hexo-theme-ayer-modified.git themes/ayer
```
...
主题自带默认配置文件 `themes/ayer/_config.yml`。你**不应直接修改**这个文件，而是在站点根目录创建 `_config.ayer.yml`，只填写需要覆盖的选项。Hexo 5+ 会自动将其与主题默认配置合并。
